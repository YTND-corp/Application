...
class PaymentMethodAdapter(private var listener: ((item: PaymentMethod, adapterPosition: Int) -> Unit)? = null) :
    ...
    inner class ViewHolder(val binding: PaymentMethodItemBinding) :
        ...
        fun bind(method: PaymentMethod): Unit = with(binding) {
            item = method.name
            isSelected = canSelected && method.name == selected?.name
            executePendingBindings()

            root.setOnClickListener {
                canSelected = true
                listener?.invoke(method, adapterPosition)
            }
        }
    }
}